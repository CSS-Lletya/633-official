...
import com.rs.game.dialogue.impl.TransportationDialogue;
...
public class ItemTeleports {
	...
	public static boolean transportationDialogue(Player player, Item item, boolean wearing) {
	...
	public static boolean transportationDialogue(Player player, int itemId, boolean wearing) {
	...
	private static boolean checkAll(Player player, Item item, int index, int optionsIndex, int optionsLength) {
		...
		if (optionsIndex >= optionsLength || optionsIndex >= COORDINATES[index].length) {
			player.getMovement().stopAll(); // nowhere option
			return false;
		}
	...
	public static void sendItemTransport(Player player, Item item, boolean wearing, int locationIndex) {
		if (!wearing)
			ItemTeleports.transportationDialogue(player, new Item(item.getId(), 1), wearing);
		else
	...
	public static void sendTeleport(Player player, Item item, int optionIndex) {
		...
		if (index < 0)
			return;
		sendTeleport(player, item, index, false);
	}
