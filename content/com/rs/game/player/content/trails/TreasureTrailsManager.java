...
public class TreasureTrailsManager {
    ...
    private static int getScrollboxLevel(int id) {
    ...
    private ClueDetails generateClueDetails(int dificulty) {
        while (true) {
             ...
             * detail; else if (difference == 1 && RandomUtils.random(2) == 0) return detail; else
             * if (difference == 2 && RandomUtils.random(4) == 0) return detail; else if
             * (difference == 3 && RandomUtils.random(8) == 0) return detail;
             */
        ...
        if (cluePhase == lastPhase) {
            ...
            if (source == SOURCE_DIG) {
                ...
                if (!currentClue.isLast()) {
                    ...
                    player.dialogue(d -> d.item(SCROLL_BOXES[currentClue.dificulty], "You've found a scroll box!"));
                    player.getPackets().sendGameMessage("You've found a scroll box!");
                } else {
                    player.getInventory().addItem(REWARD_CASKETS[currentClue.dificulty], 1);
                    player.dialogue(d -> d.item(3600, "You've found a reward casket!"));
                    player.getPackets().sendGameMessage("You've found a reward casket!");
                ...
                currentClue.details = generateClueDetails(currentClue.dificulty);
                player.dialogue(d -> d.item(CLUE_SCROLLS[currentClue.dificulty], NPCDefinitions.getNPCDefinitions(id).getName() + " has given you another clue scroll."));
                player.getPackets().sendGameMessage(NPCDefinitions.getNPCDefinitions(id).getName() + " has given you another clue scroll.");
                ...
                if (!currentClue.isLast())
                    currentClue.count--;
                currentClue.details = generateClueDetails(currentClue.dificulty);
                player.dialogue(d -> d.item(SCROLL_BOXES[currentClue.dificulty], "You've been given a scroll box!"));
                player.getPackets().sendGameMessage("You've been given a scroll box!");
                ...
                if (!currentClue.isLast()) {
                    ...
                    currentClue.details = generateClueDetails(currentClue.dificulty);
                    player.getInventory().addItem(SCROLL_BOXES[currentClue.dificulty], 1);
                    player.dialogue(d -> d.item(CLUE_SCROLLS[currentClue.dificulty], "You've found another clue!"));
                    player.getPackets().sendGameMessage("You've found another clue!");
                } else {
                    player.getInventory().addItem(REWARD_CASKETS[currentClue.dificulty], 1);
                    player.dialogue(d -> d.item(3600, "You've found a reward casket!"));
                    player.getPackets().sendGameMessage("You've found a reward casket!");
            ...
            if (cluePhase == 0 && (currentClue.details.type == COORDINATE || currentClue.details.type == EMOTE)
                ...
                boolean inWilderness = WildernessMapZone.isAtWild(player);
                ...
                final ClueNPC npc = new ClueNPC(player,
                        ...
                        Utility.getFreeTile(player, 1), -1, true);
                ...
                final NPC npc = new Ugi(player, 5141, Utility.getFreeTile(player, 1), -1, true);
                npc.setNextGraphics(new Graphics(74));
                World.get().submit(new Task(1) {
					@Override
					protected void execute() {
    ...
    public boolean useItem(Item item, int slot) {
        ...
        if (level != -1) {
            ...
            player.dialogue(d -> d.item(CLUE_SCROLLS[getScrollboxLevel(item.getId())],"You've found another clue!"));
            return true;
        }
            ...
            for (Item item : requiredItems) {
                ...
                if (slot == Equipment.SLOT_RING && item.getId() == 20922) {
                    if (!player.getEquipment()
                            .containsAny(2552, 2554, 2556, 2558, 2560, 2562, 2564, 2566)) {
                        return;
                    }
                } else if (requestedItem.getId() != item.getId()) {
                    player.getPackets().sendGameMessage("You're not wearing the correct items for this treasure trail.");
                    return;
                }
            }
        }
    ...
    public boolean useDig() {
        ...
        if ((currentClue.details.type == SIMPLE || currentClue.details.type == MAP)
            ...
            if (!hasCurrentClue()) {
                player.getPackets().sendGameMessage("You dont have a clue.");
                return false;
            }
            WorldTile tile = new WorldTile(currentClue.details.getId());
            if (!player.withinDistance(tile, currentClue.details.type == MAP ? 6 : 16)) {
                player.getPackets().sendGameMessage(tile.getX() + " - " + tile.getY());
                return false;
            }
            ...
            if (!player.withinDistance(t, 6)) {
                player.getPackets().sendGameMessage(t.getX() + " - " + t.getY());
                return false;
            }
            setNextClue(SOURCE_DIG);
            return true;
            ...
            if (!hasSextant(player))
                player.getPackets().sendGameMessage("You don't have a sextant.");
            if (!hasChart(player))
                player.getPackets().sendGameMessage("You don't have a chart.");
            if (!hasWatch(player))
                player.getPackets().sendGameMessage("You don't have a watch.");
        }
        return false;
    }
    ...
    public boolean useObject(GameObject object) {
        if (currentClue == null)
            return false;
        if (currentClue.details.idType != OBJECT || currentClue.details.getId() != object.getId()) {
            return false;
    ...
    public boolean useNPC(final NPC npc) {
        if (currentClue == null)
            return false;
        if (currentClue.details.idType != NPC || currentClue.details.getId() != npc.getId())
            return false;
        if (currentClue.details.type != SIMPLE && currentClue.details.type != ANAGRAM)
            return false;
        else if (!hasCurrentClue())
    ...
    public void openPuzzle(int itemId) {
        if (currentClue == null) {
            player.getPackets().sendGameMessage("clue is null");
            return;
    ...
    public void openReward(int level) {
        ...
        for (Item item : player.getClueScrollRewards().toArray()) {
            ...
            if (name.contains("third") || name.contains("ranger") || name.contains("robin") || name.contains("gilded")
                ...
                World.sendWorldMessage("<col=36648b>News: " + player.getDisplayName() + " looted "
                    ...
                    if (bank) {
                        ...
                        if (player.getInventory().hasFreeSlots())
                            ...
                            if (player.getInventory().containsItem(item.getId(), 1)
                                ...
                                FloorItem.updateGroundItem(item, player.getLastWorldTile(), player, 60, 0);
                        }
                    }
    ...
    public boolean hasChart(Player player) {
        return player.getInventory().containsOneItem(CHART);
    }

    public boolean hasSextant(Player player) {
        return player.getInventory().containsOneItem(SEXTANT);
    }

    public boolean hasWatch(Player player) {
        return player.getInventory().containsOneItem(WATCH);
    }
...
//        SIMPLE_S_8(EASY, SIMPLE, OBJECT, 11745, "Search in the basement of the", "Artisan Dwarves' workshop in", "Falador."),
        ...
        EMOTE_T_1(EASY, EMOTE, TILE, 51776739, new Emote[]{Emote.THINKING}, new Item[]{new Item(1656, 1), new Item(843, 1)}, "Think in the middle of the", "wheat field by the Lumbridge mill.", "Equip a sapphire necklace", "and an oak shortbow. ")
        // ,
        ...
        EMOTE_Y_2(EASY, EMOTE, TILE, 50482355, new Emotes.Emote[]{Emotes.Emote.YAWN}, new Item[]{new Item(1191, 1), new Item(1295, 1), new Item(1097, 1)}, "Yawn in Draynor Marketplace.", "Equip an iron kiteshield,", "steel longsword", "and studded leather chaps."),
