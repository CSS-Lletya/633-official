...
@ObjectSignature(objectId = {}, name = {"Tree", "Evergreen", "Dead tree", "Oak", "Yew", "Willow", "Maple", "Magic tree", "Teak", "Mahogany", "Ivy"})
public class TreeObjectPlugin extends ObjectListener {
	...
	public void execute(Player player, GameObject object, int optionId) throws Exception {
		object.doAction(optionId, object.getId(), "Chop down", () -> {
			...
			.filter(data -> Arrays.stream(data.getObject()).anyMatch(tree -> tree.getObjectId() == object.getId())).findFirst()
			.ifPresent(data -> new Woodcutting(player, data, object).start());
		});
		if (object.getDefinitions().getNameContaining("Ivy")) {
			Arrays.stream(Tree.values())
			.filter(data -> Arrays.stream(data.getObject()).anyMatch(tree -> tree.getObjectId() == object.getId())).findFirst()
			.ifPresent(data -> new Woodcutting(player, data, object).start());
		}
