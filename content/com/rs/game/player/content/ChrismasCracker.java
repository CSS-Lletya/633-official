...
public class ChrismasCracker {
	...
	final static Item[] EXTRA_ITEMS = { new Item(1969, 1), new Item(2355, RandomUtils.random(1, 2)), new Item(1217, 1),
	...
	public static void splitCracker(Player player, Player target, Item cracker) {
		...
		if (player.getInventory().removeItems(cracker)) {
			...
			boolean winner = RandomUtils.random(2) == 1;
			...
			Item hat = RandomUtils.random(HATS);
			if (winner) {
			...
			if (!winner) {
				player.getInventory().addItem(RandomUtils.random(EXTRA_ITEMS));
			}
		}
	}
}