...
public class AncientEffigyItemPlugin extends InventoryListener {
	...
	public void execute(Player player, Item item, int slot, int option) {
		...
		player.dialogue(d -> {
			d.mes("As you inspect the ancient effigy, you begin to feel a strange sensation of the relic searching your mind, drawing on your knowledge.");
			...
			d.option("Focus on " + Skills.SKILL_NAME[skill1], () -> {
				if (player.getSkills().getTrueLevel(skill1) < AncientEffigies.getRequiredLevel(item.getId())) {
					...
					d.event(() -> {
						...
						player.dialogue(end -> end.mes("The ancient effigy glows briefly; it seems changed somehow and no longer responds to the same memories as before."));
					});
				}
