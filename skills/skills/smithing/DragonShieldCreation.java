...
public class DragonShieldCreation {
	...
	public DragonShieldCreation craftShield(Player player) {
		...
		if (type == 1)
			player.dialogue(d -> d.mes("You set to work trying to fix the ancient shield. It's seen some heavy action and needs some serious work doing to it."));
		else
			player.dialogue(d -> d.mes("You set to work, trying to attach the ancient draconic visage to your anti-dragonbreath shield. It's not easy to work with the ancient artifact and it takes all of your skills as a master smith."));
		switch(type) {
		case 1:
