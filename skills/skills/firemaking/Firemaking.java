...
public class Firemaking extends DestructionSkillAction {
	 ...
	 */
	Firemaking(Player player, Item firstItem, Item secondItem, boolean familiar, boolean isGround, WorldTile tile) {
		...
		this.isGroundLog = isGround;
		this.toTile = tile;
	}
	
	public static boolean execute(Player player, Item firstItem, Item secondItem, boolean familiar, boolean isGround, WorldTile tile) {
		Firemaking firemaking = new Firemaking(player, firstItem, secondItem, familiar, isGround, tile);
		if(familiar) {
			firemaking.start();
			return true;
		}
